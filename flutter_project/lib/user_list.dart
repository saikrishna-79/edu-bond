...
class UserListPage extends StatefulWidget {
  ...
  UserListPage({required this.searchValue, required this.fromUsername});
...
class _UserListPageState extends State<UserListPage> {
  ...
  Future<void> fetchUsers() async {
    try {
      final course = widget.searchValue;
      final learningResponse = await http.get(Uri.parse(
          'http://192.168.56.1:3000/users/learning?course=$course&username=${widget.fromUsername}'));
      final learntResponse = await http.get(Uri.parse(
          'http://192.168.56.1:3000/users/learnt?course=$course&username=${widget.fromUsername}'));
                            ...
                            Expanded(
                              child: learntUsers.isEmpty
                                  ? Padding(
                                      padding: const EdgeInsets.all(8.0),
                                      child: Text(
                                        'No users have completed learning this course.',
                                        style: TextStyle(color: Colors.grey),
                                        textAlign: TextAlign.center,
                                      ),
                                    )
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            TextButton(
              onPressed: () {
                print(_optionalCourseController.text);
  ...
  Future<void> _requestBond(String toUsername, String courseFromPreviousPage,
    ...
    final response = await http.post(
      ...
      body: json.encode({
        ...
        'course': widget.searchValue, // Course from the previous page
        'request_text':
